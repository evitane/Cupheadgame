		...
		class Asteroid
		{
			...
			*/
			constructor()
			{
				...
				this.explosionSound = createAudio("game-assets/damage.wav");
				this.detonationSound = createAudio("game-assets/detonation.wav");
			...
			load()
			{
				...
				*/
				this.x = Math.floor(Math.random() * 800); // random X position
			...
			detonate()
			{
				if (!this.exploded) // each asteroid can explode/detonate only once
				{
					this.exploded = true;
					this.image = this.explosionImage; // change the asteroid image with explosion
					this.detonationSound.play(); // explosion sound
		...
		class MissilePack
		{
			...
			constructor()
			{
				...
				this.loadSound = createAudio("game-assets/charge.wav");
			}
		...
		class SpaceShip
		{
			...
			constructor()
			{
				...
				this.engineSound = createAudio("game-assets/carnival.wav");
			}
			...
			move(move)
			{
				...
				if (this.x < 800 && move > 0) // chech that will not get out of the right barrier
				{
					this.x += (move*10);
				}
			}
		...
		*/
		function preload() 
		{
			...
			gameSounds 
			$.get('game-assets/carnival.pd', function(patchStr) {
		...
		function keyPressed()
		{
			...
			if (keyCode == 83) // s is pressed - New game
			{
				startGame = true;
				startOnce = true;
				gameOver = false;
		...
		function showMessages()
		{
			...
			{
				text('Press S to start a new game!', 300, 300, 800, 200);
				textSize(20);
				text('Use the left and right arrows to avoid the falling balls.', 340, 360, 800, 200);
				text('Get the water power ups and fire by pressing the Space bar!', 340, 390, 800, 200);
			}
			
			if (gameOver)
			{
				textSize(40);
				text('Rest In Peace!', 500, 280);
			}
			
			if (paused)
			{
				textSize(40);
				text('Are you scared?!', 500, 280);
			}
		}
